...
import { getRandomLyricPassage } from "./data/lyrics";

const MAX_TRAVEL_VW = 60; // approximate travel distance toward the bowl

function App() {
  const initialLyricRef = useRef(getRandomLyricPassage(30));
  const [lyricMeta, setLyricMeta] = useState({
    artist: initialLyricRef.current.artist,
    title: initialLyricRef.current.title,
  });
  const [words, setWords] = useState(() =>
    initialLyricRef.current.line.split(/\s+/)
  );
  ...
  const handleWordComplete = (correct, typedWord) => {
    if (!correct) return;
    const wordLen =
      typeof typedWord === "string" ? typedWord.length : expectedWord.length;
    // include trailing space as a typed character
    charsTypedRef.current += wordLen + 1;
    const nextIndex = currentIndex + 1;
    setCurrentIndex(nextIndex);
    if (startedRef.current) {
      const elapsedMs = Date.now() - startTimeRef.current;
      const minutes = elapsedMs / 60000;
      const grossWpm = minutes > 0 ? charsTypedRef.current / 5 / minutes : 0;
  ...
  const resetRace = () => {
    const next = getRandomLyricPassage(30);
    setLyricMeta({ artist: next.artist, title: next.title });
    setWords(next.line.split(/\s+/));
    setCurrentIndex(0);
    setWpm(0);
    setTyped("");
        ...
        {isFinished && (
          ...
          >
            ...
            {lyricMeta && (
              ...
              >
                {lyricMeta.artist} — {lyricMeta.title}
              </div>
            )}
          </div>
        )}
